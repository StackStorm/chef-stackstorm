...
file "creating #{conf.home}/.ssh/authorized_keys for #{conf.user}" do
  ...
  content lazy {
    ...
    self_pub_key = ::IO.read("#{conf.home}/.ssh/id_rsa.pub")
    pub_keys = node['stackstorm']['user']['authorized_keys'].dup
    pub_keys.unshift(self_pub_key)
